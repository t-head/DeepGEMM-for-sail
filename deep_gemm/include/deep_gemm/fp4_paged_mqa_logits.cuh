...
// ============================================================================
// Key differences from FP8 Sm80PagedMqaLogits:
...
#pragma unroll
                    ...
                    auto transform = [&](__nv_bfloat162 sum, int j, int n) -> __nv_bfloat162 {
                        // __nv_bfloat16 a0 = fmaxf(accum(j,m,n), 0);
                        // __nv_bfloat16 a1 = fmaxf(accum(j+1,m,n), 0);
                        // __nv_bfloat162 a = {a0, a1};

                        __nv_bfloat162 a = {(__nv_bfloat16)accum(j, m, n), (__nv_bfloat16)accum(j + 1, m, n)};
                        a = __hmax2(a, {0, 0});
                        __nv_bfloat162 b = {tCrW(j, n), tCrW(j + 1, n)};
                        return __hfma2(a, b, sum);
                    };
                    __nv_bfloat162 sum_0 = {0, 0};
                    __nv_bfloat162 sum_1 = {0, 0};
#pragma unroll
                    for (int n = 0; n < size<2>(accum); n++) {
                        sum_0 = transform(sum_0, 0, n);
                        sum_1 = transform(sum_1, 2, n);
                        sum_0 = transform(sum_0, 4, n);
                        sum_1 = transform(sum_1, 6, n);
                    }
                    // Intra-thread reduction in bfloat16
                    __nv_bfloat16 v_0_bf = __hadd(__low2bfloat16(sum_0), __high2bfloat16(sum_0));
                    __nv_bfloat16 v_1_bf = __hadd(__low2bfloat16(sum_1), __high2bfloat16(sum_1));

                    // Cross-lane reduction: pack v_0/v_1 into bfloat162 for single 32-bit shuffle
                    __nv_bfloat162 packed = {v_0_bf, v_1_bf};
#pragma unroll
                    for (int j = 0; j < 2; ++j) {
                        uint32_t bits = reinterpret_cast<uint32_t&>(packed);
                        uint32_t received_bits = __shfl_xor_sync(0xffffffffu, bits, 1u << j);
                        __nv_bfloat162 received = reinterpret_cast<__nv_bfloat162&>(received_bits);
...
public:
    static void run(const ElementQK* ptr_q, const uint32_t* q_sf, const ElementQK* ptr_k, const uint32_t* k_sf,
        ...
        using AttnKernel =
            cutlass::gemm::kernel::Sm80PagedMqaLogitsFP4<ElementQK, ElementAcc, ElementLogits, kNextN, kNumHeads,
        ...
        if (ProfilingInterface::Instance().get_op_info()) {
            dg_prof_params.set_paged_mqa_logits_params("fp4", batch_size, kNextN, kNumHeads, kHeadDim * 2,
                                                       ...
                                                       stream);
            if constexpr (std::is_same_v<ElementLogits, __nv_bfloat16>) {
