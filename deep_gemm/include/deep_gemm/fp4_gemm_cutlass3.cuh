...
{
    constexpr uint32_t stripe_capacity_int4s = K_TILES_PER_FLAG * BLOCK_K / 16;
    constexpr bool kRaggedKstripe =
        K_TILES_PER_FLAG == 8 || K_TILES_PER_FLAG == 16;

    const uint32_t num_copy = sched.num_copy_blocks;
    const uint32_t k_half = sched.local_buf_k_half;
    const uint32_t k_half_int4s = k_half / 16;
    const uint32_t max_tok = sched.local_buf_max_tokens;
    const uint32_t num_stripes = kRaggedKstripe
        ? (k_half_int4s + stripe_capacity_int4s - 1) / stripe_capacity_int4s
        : k_half_int4s / stripe_capacity_int4s;
    constexpr uint32_t nr = NumRanks;
    const uint32_t stride = blockDim.x;
    ...
    for (uint32_t mb = blockIdx.x; mb < total_m_blocks; mb += num_copy) {
        ...
        for (uint32_t ks = 0; ks < num_stripes; ++ks) {
            const uint32_t stripe_col_start = ks * stripe_capacity_int4s;
            const uint32_t stripe_int4s = kRaggedKstripe
                ? min(stripe_capacity_int4s, k_half_int4s - stripe_col_start)
                : stripe_capacity_int4s;
...
#ifdef DG_BULK_COPY
                if (mb < 2) {
                    // Generic (non-remote) PPU bulk load/store for the wave0-critical
                    // M-blocks. Bulk is warp-collective, so keep every participating
                    // warp on one contiguous 32*int4 == 512-byte chunk within a token
                    // row. Full 512-byte chunks use bulk operations; any partial
                    // row tail stays on plain loads, as do rank-count-ragged rows
                    // below min_n.
                    constexpr uint32_t kBulkLanes = 32;
                    const uint32_t kBulkChunksPerRow = stripe_int4s / kBulkLanes;
                    const uint32_t kBulkTailPerRow = stripe_int4s % kBulkLanes;
                    const uint32_t common_rows = min_n / stripe_int4s;
                    const uint32_t lane = threadIdx.x & (kBulkLanes - 1);
                    const uint32_t warp = threadIdx.x / kBulkLanes;
                    const uint32_t num_warps = blockDim.x / kBulkLanes;

                    if (kBulkChunksPerRow > 0) {
                        const uint32_t total_chunks = common_rows * kBulkChunksPerRow;
                        uint32_t chunk = warp;
#ifdef DG_BULK_PIPE2
                        // Keep two independent full-warp chunks in flight. Each
                        // chunk contributes eight rank loads, so the paired body
                        // exposes 16 bulk loads before the first dependent
                        // store/un-swizzle. Dynamic-routing shapes that leave only
                        // one chunk for a warp fall through to the single body.
#ifdef DG_BULK_ROLL2
                        // Fuse two 16-load windows. A side-effect compiler marker
                        // (without a memory clobber) keeps each drain/refill pair
                        // visible to instruction scheduling without lowering to
                        // the all-counter s.wait generated by a memory clobber.
                        // ISA audit is mandatory because the backend may still hoist.
                        for (; chunk + 3 * num_warps < total_chunks;
                             chunk += 4 * num_warps) {
                            const uint32_t row = chunk / kBulkChunksPerRow;
                            const uint32_t chunk_in_row =
                                chunk - row * kBulkChunksPerRow;
                            const uint32_t col =
                                chunk_in_row * kBulkLanes + lane;
                            const uint32_t addr =
                                row * k_half_int4s + stripe_col_start + col;
                            const uint32_t next_chunk = chunk + num_warps;
                            const uint32_t next_row =
                                next_chunk / kBulkChunksPerRow;
                            const uint32_t next_chunk_in_row =
                                next_chunk - next_row * kBulkChunksPerRow;
                            const uint32_t next_col =
                                next_chunk_in_row * kBulkLanes + lane;
                            const uint32_t next_addr =
                                next_row * k_half_int4s + stripe_col_start + next_col;

                            int4 v0 = ld_bulk_global(s0 + addr);
                            int4 v1 = ld_bulk_global(s1 + addr);
                            int4 v2 = ld_bulk_global(s2 + addr);
                            int4 v3 = ld_bulk_global(s3 + addr);
                            int4 v4 = ld_bulk_global(s4 + addr);
                            int4 v5 = ld_bulk_global(s5 + addr);
                            int4 v6 = ld_bulk_global(s6 + addr);
                            int4 v7 = ld_bulk_global(s7 + addr);
                            int4 w0 = ld_bulk_global(s0 + next_addr);
                            int4 w1 = ld_bulk_global(s1 + next_addr);
                            int4 w2 = ld_bulk_global(s2 + next_addr);
                            int4 w3 = ld_bulk_global(s3 + next_addr);
                            int4 w4 = ld_bulk_global(s4 + next_addr);
                            int4 w5 = ld_bulk_global(s5 + next_addr);
                            int4 w6 = ld_bulk_global(s6 + next_addr);
                            int4 w7 = ld_bulk_global(s7 + next_addr);

                            // Scalar address work is intentionally placed after
                            // the first 16 issues so it overlaps their latency.
                            const uint32_t third_chunk =
                                chunk + 2 * num_warps;
                            const uint32_t third_row =
                                third_chunk / kBulkChunksPerRow;
                            const uint32_t third_chunk_in_row =
                                third_chunk - third_row * kBulkChunksPerRow;
                            const uint32_t third_col =
                                third_chunk_in_row * kBulkLanes + lane;
                            const uint32_t third_addr =
                                third_row * k_half_int4s + stripe_col_start + third_col;
                            const uint32_t fourth_chunk =
                                chunk + 3 * num_warps;
                            const uint32_t fourth_row =
                                fourth_chunk / kBulkChunksPerRow;
                            const uint32_t fourth_chunk_in_row =
                                fourth_chunk - fourth_row * kBulkChunksPerRow;
                            const uint32_t fourth_col =
                                fourth_chunk_in_row * kBulkLanes + lane;
                            const uint32_t fourth_addr =
                                fourth_row * k_half_int4s + stripe_col_start + fourth_col;

#define DG_BULK_DRAIN_REFILL(OLD_V, OLD_D, OLD_ADDR, NEW_V, NEW_S, NEW_ADDR) \
                            st_bulk_global(OLD_D + OLD_ADDR, OLD_V);          \
                            asm volatile("");                                  \
                            int4 NEW_V = ld_bulk_global(NEW_S + NEW_ADDR);    \
                            asm volatile("")
                            DG_BULK_DRAIN_REFILL(v0, d0, addr, x0, s0, third_addr);
                            DG_BULK_DRAIN_REFILL(v1, d1, addr, x1, s1, third_addr);
                            DG_BULK_DRAIN_REFILL(v2, d2, addr, x2, s2, third_addr);
                            DG_BULK_DRAIN_REFILL(v3, d3, addr, x3, s3, third_addr);
                            DG_BULK_DRAIN_REFILL(v4, d4, addr, x4, s4, third_addr);
                            DG_BULK_DRAIN_REFILL(v5, d5, addr, x5, s5, third_addr);
                            DG_BULK_DRAIN_REFILL(v6, d6, addr, x6, s6, third_addr);
                            DG_BULK_DRAIN_REFILL(v7, d7, addr, x7, s7, third_addr);
                            DG_BULK_DRAIN_REFILL(w0, d0, next_addr, y0, s0, fourth_addr);
                            DG_BULK_DRAIN_REFILL(w1, d1, next_addr, y1, s1, fourth_addr);
                            DG_BULK_DRAIN_REFILL(w2, d2, next_addr, y2, s2, fourth_addr);
                            DG_BULK_DRAIN_REFILL(w3, d3, next_addr, y3, s3, fourth_addr);
                            DG_BULK_DRAIN_REFILL(w4, d4, next_addr, y4, s4, fourth_addr);
                            DG_BULK_DRAIN_REFILL(w5, d5, next_addr, y5, s5, fourth_addr);
                            DG_BULK_DRAIN_REFILL(w6, d6, next_addr, y6, s6, fourth_addr);
                            DG_BULK_DRAIN_REFILL(w7, d7, next_addr, y7, s7, fourth_addr);
#undef DG_BULK_DRAIN_REFILL

                            st_bulk_global(d0 + third_addr, x0);
                            st_bulk_global(d1 + third_addr, x1);
                            st_bulk_global(d2 + third_addr, x2);
                            st_bulk_global(d3 + third_addr, x3);
                            st_bulk_global(d4 + third_addr, x4);
                            st_bulk_global(d5 + third_addr, x5);
                            st_bulk_global(d6 + third_addr, x6);
                            st_bulk_global(d7 + third_addr, x7);
                            st_bulk_global(d0 + fourth_addr, y0);
                            st_bulk_global(d1 + fourth_addr, y1);
                            st_bulk_global(d2 + fourth_addr, y2);
                            st_bulk_global(d3 + fourth_addr, y3);
                            st_bulk_global(d4 + fourth_addr, y4);
                            st_bulk_global(d5 + fourth_addr, y5);
                            st_bulk_global(d6 + fourth_addr, y6);
                            st_bulk_global(d7 + fourth_addr, y7);
                        }
#endif
                        for (; chunk + num_warps < total_chunks;
                             chunk += 2 * num_warps) {
                            const uint32_t row = chunk / kBulkChunksPerRow;
                            const uint32_t chunk_in_row = chunk - row * kBulkChunksPerRow;
                            const uint32_t col = chunk_in_row * kBulkLanes + lane;
                            const uint32_t addr = row * k_half_int4s + stripe_col_start + col;
                            const uint32_t next_chunk = chunk + num_warps;
                            const uint32_t next_row = next_chunk / kBulkChunksPerRow;
                            const uint32_t next_chunk_in_row =
                                next_chunk - next_row * kBulkChunksPerRow;
                            const uint32_t next_col =
                                next_chunk_in_row * kBulkLanes + lane;
                            ...
                            int4 w6 = ld_bulk_global(s6 + next_addr);
                            int4 w7 = ld_bulk_global(s7 + next_addr);
                            st_bulk_global(d0 + addr, v0); st_bulk_global(d1 + addr, v1);
                            st_bulk_global(d2 + addr, v2); st_bulk_global(d3 + addr, v3);
                            st_bulk_global(d4 + addr, v4); st_bulk_global(d5 + addr, v5);
                            st_bulk_global(d6 + addr, v6); st_bulk_global(d7 + addr, v7);
                            st_bulk_global(d0 + next_addr, w0); st_bulk_global(d1 + next_addr, w1);
                            st_bulk_global(d2 + next_addr, w2); st_bulk_global(d3 + next_addr, w3);
                            st_bulk_global(d4 + next_addr, w4); st_bulk_global(d5 + next_addr, w5);
                            st_bulk_global(d6 + next_addr, w6); st_bulk_global(d7 + next_addr, w7);
                        }
#endif
                        // One full chunk can remain per warp under dynamic routing
                        // and in the final stripe; keep that remainder on the
                        // shallower eight-load body.
                        for (; chunk < total_chunks; chunk += num_warps) {
                            const uint32_t row = chunk / kBulkChunksPerRow;
                            const uint32_t chunk_in_row = chunk - row * kBulkChunksPerRow;
                            const uint32_t col = chunk_in_row * kBulkLanes + lane;
                            const uint32_t addr = row * k_half_int4s + stripe_col_start + col;
                    ...
                    if (kBulkTailPerRow > 0) {
                        const uint32_t total_tail = common_rows * kBulkTailPerRow;
                        for (uint32_t t = threadIdx.x; t < total_tail; t += stride) {
                            const uint32_t row = t / kBulkTailPerRow;
                            const uint32_t col = kBulkChunksPerRow * kBulkLanes +
                                (t - row * kBulkTailPerRow);
                            const uint32_t addr = row * k_half_int4s + stripe_col_start + col;
                    ...
                    // Resume the per-thread ragged-rank tail at the first
                    // logical element after the uniform bulk-covered rows.
                    i = min_n + threadIdx.x;
                }
#endif
#ifdef DG_COPY_PIPE2
                if (mb < 2) {
                    // Wave0 consumes mb0 and mb1 first. Keep two independent
                    // eight-rank groups live so the backend can issue up to 16
                    // vector loads before the dependent stores; later M-blocks
                    // retain the original lower-pressure schedule.
                    for (; i + stride < min_n; i += 2 * stride) {
                        uint32_t row0 = i / stripe_int4s;
                        uint32_t col0 = i % stripe_int4s;
                        uint32_t addr0 = row0 * k_half_int4s + stripe_col_start + col0;
                        uint32_t i1 = i + stride;
                        uint32_t row1 = i1 / stripe_int4s;
                        uint32_t col1 = i1 % stripe_int4s;
                        uint32_t addr1 = row1 * k_half_int4s + stripe_col_start + col1;

                        int4 v00 = ld_nc_global(s0 + addr0);
                        int4 v01 = ld_nc_global(s1 + addr0);
                        int4 v02 = ld_nc_global(s2 + addr0);
                        int4 v03 = ld_nc_global(s3 + addr0);
                        int4 v04 = ld_nc_global(s4 + addr0);
                        int4 v05 = ld_nc_global(s5 + addr0);
                        int4 v06 = ld_nc_global(s6 + addr0);
                        int4 v07 = ld_nc_global(s7 + addr0);
                        int4 v10 = ld_nc_global(s0 + addr1);
                        int4 v11 = ld_nc_global(s1 + addr1);
                        int4 v12 = ld_nc_global(s2 + addr1);
                        int4 v13 = ld_nc_global(s3 + addr1);
                        int4 v14 = ld_nc_global(s4 + addr1);
                        int4 v15 = ld_nc_global(s5 + addr1);
                        int4 v16 = ld_nc_global(s6 + addr1);
                        int4 v17 = ld_nc_global(s7 + addr1);
                        asm volatile("" ::: "memory");

                        d0[addr0] = v00; d1[addr0] = v01;
                        d2[addr0] = v02; d3[addr0] = v03;
                        d4[addr0] = v04; d5[addr0] = v05;
                        d6[addr0] = v06; d7[addr0] = v07;
                        d0[addr1] = v10; d1[addr1] = v11;
                        d2[addr1] = v12; d3[addr1] = v13;
                        d4[addr1] = v14; d5[addr1] = v15;
                        d6[addr1] = v16; d7[addr1] = v17;
                    }
                    if (i < min_n) {
                        ...
                        int4 v7 = ld_nc_global(s7 + addr);
                        asm volatile("" ::: "memory");
                        d0[addr] = v0; d1[addr] = v1;
                        d2[addr] = v2; d3[addr] = v3;
                        d4[addr] = v4; d5[addr] = v5;
                        d6[addr] = v6; d7[addr] = v7;
                        i += stride;
                    }
                }
#endif
...
public:
    ...
    static void run_fused_dispatch(
        ...
        static_assert(KTilesPerFlag == 0 ||
                      (ShapeK / BlockK) % KTilesPerFlag == 0 ||
                      KTilesPerFlag == 8 || KTilesPerFlag == 16,
                      "KTilesPerFlag must divide num_k_tiles or select ragged "
                      "KTPF8/KTPF16");
        constexpr int N_EXPAND = NExpand;
