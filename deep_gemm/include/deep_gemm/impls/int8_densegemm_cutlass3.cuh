...
/// \file int8_densegemm_cutlass3.cuh
/// \brief Standalone INT8 DenseGemm kernel — SHAPE_N/SHAPE_K are runtime arguments.
/// Extracted from int8_gemm_cutlass3.cuh, removing MoeDynamicTile, GroupedMasked,
/// and all non-DenseGemm paths.

// Self-contained INT8 DenseGemm header: the MainloopPPUAiuA8W8 dispatch policy
// and its CollectiveMma specialization are copied verbatim below (from
// int8_gemm_cutlass3.cuh) so this header no longer depends on the group-GEMM
// header. The direct underlying includes below mirror int8_gemm_cutlass3.cuh.
#ifndef INT8_HGRTC
...
#include <deep_gemm/common/warp_on_k_reduction.cuh>
...
////////////////////////////////////////////////////////////////////////////////
// MainloopPPUAiuA8W8 dispatch policy + CollectiveMma specialization
// (copied verbatim from int8_gemm_cutlass3.cuh; group GEMM keeps its own copy).
////////////////////////////////////////////////////////////////////////////////
...
struct CollectiveMma<
    Arch_,
    MainloopPPUAiuA8W8<Stages, KernelSchedule, DenseS2Opt>,
    TileShape_,
    ElementA_,
    StrideA_,
    ElementB_,
  ...
  // Type Aliases
  //
  using DispatchPolicy = MainloopPPUAiuA8W8<Stages, KernelSchedule, DenseS2Opt>;
  ...
  // AIU N-consolidation: merge K-direction AIU loads into one instruction
  static constexpr int AIU_CUBE_W = 128;           // 128 int8 elements = 128 bytes per AIU slab
  static constexpr int AIU_CUBE_N = CTA_K / AIU_CUBE_W;  // Number of K-slabs to consolidate
  ...
  operator() (
    ...
    // When WarpOnK > 1, K>0 warps must read the same scale factor positions as K=0 warps
    // because scale factors only have MN dimensions (no K dimension).
    // The tiled_copy_C thread layout includes WarpOnK, causing K>0 threads to map to
    // out-of-range SMEM positions. Remap thread_idx to strip the K-warp contribution.
    constexpr int kThreadsPerMN_scale = CUTE_STATIC_V(size(TiledMma{})) / CUTE_STATIC_V(size<3>(typename TiledMma::ThrLayoutVMNK{}));
    int thread_idx_for_scale = thread_idx % kThreadsPerMN_scale;
    auto smem_thr_copy_ScaleA     = smem_tiled_copy_ScaleA.get_thread_slice(thread_idx_for_scale);
    ...
    auto smem_thr_copy_ScaleB     = smem_tiled_copy_ScaleB.get_thread_slice(thread_idx_for_scale);
...
/// Simplified DeepGemmUniversal for DenseGemm only.
...
////////////////////////////////////////////////////////////////////////////////
// DenseGemm host-side wrapper class
////////////////////////////////////////////////////////////////////////////////
...
class DenseGemm {
public:
    static_assert(kKernelType == KernelType::Default, "int8 DenseGemm only supports the Default kernel type");

    DenseGemm() = default;

    static void run(__ppu_bfloat16* gmem_d,
                    uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
                    ElementAB* gmem_a, float* scales_a,
        ...
        using DispatchPolicy = cutlass::gemm::MainloopPPUAiuA8W8<kNumStages, KernelSchedule, kDenseS2Opt>;
        ...
        if (ProfilingInterface::Instance().get_op_info()) {
            dg_prof_params.set_params(
                GemmType::DenseGemm, false, std::string("int8"), 1, shape_m, shape_n, shape_k, shape_m,
                nullptr, stream
            );
        }
        ...
        if (pEnv_params && isdigit(*pEnv_params)) {
            printf("[DenseGemm-A8W8-Standalone:]\n");
