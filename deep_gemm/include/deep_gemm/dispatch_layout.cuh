...
struct DispatchBufferLayout {
    ...
    // DG_SFA_PUSH_COUNTS: a tiny counts region at the END of the SEPARATE staging buffer
    // (after the 2 SFA parities). Quant's last block pushes each (src_rank, local_expert)
    // token count here so the owner's expert_preprocess reads it locally instead of a
    // remote NVLink read. One parity = num_total_experts uint32 (== num_ranks * real local
    // experts == the full pair_counts array), x2 for parity. Offsets are relative to the
    // staging buffer base (staging_addrs[rank]), which starts at 0 (separate buffer).
    __host__ __device__
    uint64_t staging_counts_offset() const {
        return 2 * staging_parity_bytes();  // after both SFA parities
    }
    __host__ __device__
    uint64_t staging_counts_parity_bytes() const {
        return uint64_t(num_total_experts) * sizeof(uint32_t);
    }
    __host__ __device__
    uint64_t staging_bytes_total() const {
        // Separate staging buffer total: 2 SFA parities + 2 counts parities.
        return 2 * staging_parity_bytes() + 2 * staging_counts_parity_bytes();
    }

    __host__ __device__
    uint64_t total_bytes_with_staging() const {
        return staging_offset() + staging_bytes_total();
    }
