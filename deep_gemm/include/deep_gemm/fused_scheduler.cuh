...
public:
  ...
  __device__ __forceinline__ explicit FusedGemmScheduler(const int32_t* aligned_num_m_blocks,
                                                         const int32_t* m_rows,
                                                         const int32_t* expert_ids_and_cumsum)
    : m_row_indices_(m_rows), expert_ids_and_cumsum_((uint2*)expert_ids_and_cumsum), current_iter(0),
  ...
  __device__ __forceinline__ bool fetch_next_work(uint32_t& m_block_idx, uint32_t& n_block_idx) {
    ...
    uint2 data = __ldg(expert_ids_and_cumsum_ + block_m_idx);
    curr_group_idx = data.x;
    curr_group_m = __ldg(m_row_indices_ + curr_group_idx);
    uint32_t num_m_blocks = ceil_div(curr_group_m, BLOCK_M);
    get_swizzled_block_idx(num_m_blocks, next_block_idx - data.y * kNumNBlocks, m_block_idx, n_block_idx);
    cumsum_m_block_idx = m_block_idx + data.y;
    return true;
  }
...
public:
  ...
  const int32_t* m_row_indices_;
  const uint2* expert_ids_and_cumsum_;
