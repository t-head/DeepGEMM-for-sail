...
launch_mxfp4_quantize<{HIDDEN}>(
    ...
    stream, reinterpret_cast<int64_t*>(profile_clocks), staging_addrs,
...
launch_dispatch_expert_preprocess_merged<
    {BLOCK_M}, {RANK_IDX}, {NUM_RANKS}, {NUM_LOCAL_EXPERTS},
    {NUM_TOTAL_EXPERTS}, {MAX_TOKENS_PER_EXPERT}, {HIDDEN},
    {LOCAL_EXPERT_START}, {PROFILE_ENABLED}>(
    sym_buf_addrs,
    {RANK_IDX}, {NUM_RANKS},
    {NUM_LOCAL_EXPERTS}, {NUM_TOTAL_EXPERTS}, {MAX_TOKENS_PER_EXPERT}, {HIDDEN},
    {LOCAL_EXPERT_START},
...
template_expert_sfa_overlap = """
using namespace deep_gemm;

launch_dispatch_expert_sfa_overlap<
    {BLOCK_M}, {RANK_IDX}, {NUM_RANKS}, {NUM_LOCAL_EXPERTS},
    {NUM_TOTAL_EXPERTS}, {MAX_TOKENS_PER_EXPERT}, {HIDDEN},
    {LOCAL_EXPERT_START}>(
    sym_buf_addrs, generation,
    reinterpret_cast<uint32_t*>(pair_counts),
    grouped_layout,
    reinterpret_cast<uint64_t*>(rank_addr_a),
    reinterpret_cast<uint64_t*>(rank_addr_sfa),
    ...
    reinterpret_cast<uint32_t*>(copy_ready_flags),
    num_copy_ready_flags,
    reinterpret_cast<const int64_t*>(staging_addrs),
    stream);
"""
...
def get_sfa_staging_size(
    ...
    # DG_SFA_PUSH_COUNTS: tiny counts region after the 2 SFA parities (2 * num_total * 4B,
    # ~768B). Always reserved so the buffer size matches DispatchBufferLayout regardless of
    # whether counts push is enabled at runtime; the ~KB growth is far below the per-buffer
    # size threshold that would slow the staging buffer's own remote reads.
    counts_region = 2 * num_total_experts * 4
    return 2 * staging_parity + counts_region
...
def set_sfa_staging_addrs(addrs, local_base):
    ...
    if not isinstance(addrs, torch.Tensor):
        raise TypeError("SFA staging addresses must be a torch.Tensor")
    if addrs.dtype != torch.int64 or not addrs.is_contiguous():
        raise ValueError("SFA staging addresses must be contiguous int64")
    _sfa_staging_addrs = addrs
    _sfa_staging_local_base = int(local_base)


def _resolve_sfa_staging_addrs(staging_addrs, device, num_ranks, *, required):
    """Resolve an explicit staging address tensor, with the module global as legacy fallback."""
    if num_ranks <= 1:
        return torch.zeros(1, dtype=torch.int64, device=device)
    if staging_addrs is None:
        staging_addrs = _sfa_staging_addrs
    if staging_addrs is None:
        if required:
            raise RuntimeError(
                "DG_SFA_PUSH requires a symmetric SFA staging buffer; pass "
                "sfa_staging_buf and sfa_staging_addrs to BlockCopyDispatchContext")
        return torch.zeros(1, dtype=torch.int64, device=device)
    if staging_addrs.dtype != torch.int64 or not staging_addrs.is_contiguous():
        raise ValueError("SFA staging addresses must be contiguous int64")
    if staging_addrs.device != torch.device(device):
        raise ValueError(
            f"SFA staging addresses are on {staging_addrs.device}, expected {device}")
    if staging_addrs.numel() < num_ranks:
        raise ValueError(
            f"SFA staging addresses contain {staging_addrs.numel()} ranks, "
            f"expected at least {num_ranks}")
    return staging_addrs
...
def _mxfp4_quantize_to_sym_buffer(
    ...
    rank_idx: int = 0,
    num_ranks: int = 1,
    profile_clocks: torch.Tensor = None,
    staging_addrs: torch.Tensor = None,
) -> None:
    ...
    assert 0 < topk <= 8, "MXFP4 symmetric-buffer quantization supports topk in [1, 8]"
    ...
    staging_addrs = _resolve_sfa_staging_addrs(
        staging_addrs, input_bf16.device, num_ranks,
        required=(os.getenv('DG_SFA_PUSH', '0') != '0' and generation > 0))

    # profile_clocks: optional int64[3] — block0 records [Phase1 quantize,
    # Phase2 scatter/push plus the reuse barrier, total through P2 completion] cycles.
    # The explicit enable bit keeps the dummy pointer entirely inactive.
    profile_enabled = profile_clocks is not None
    if profile_clocks is None:
        # profile_enabled=0 makes this pointer entirely inactive in the kernel.
        # Reuse an already-live int64 device tensor instead of launching an
        # aten::zeros elementwise kernel for a one-element dummy on every call.
        profile_clocks = sym_buf_addrs
    else:
        min_profile_clocks = (
            7 if os.getenv('DG_QUANT_PROFILE_P1SPLIT', '0') != '0' else 3)
        if (profile_clocks.dtype != torch.int64
                or not profile_clocks.is_contiguous()
                or profile_clocks.numel() < min_profile_clocks):
            raise ValueError(
                "profile_clocks must be a contiguous int64 tensor with "
                f">= {min_profile_clocks} elements")
    if profile_clocks.device != input_bf16.device:
        raise ValueError(
            f"profile_clocks is on {profile_clocks.device}, expected {input_bf16.device}")

    args = (input_bf16, topk_ids, sym_buf,
            num_tokens, topk,
            generation,
            sym_buf_addrs, rank_idx, num_ranks,
            staging_addrs, profile_clocks, int(profile_enabled),
...
def create_expert_preprocess_workspace(
    ...
    return ExpertPreprocessWorkspace(
        ...
        masked_m=masked_m,
        # BLOCK_M-independent cache, converted by prepare to inclusive rank
        # prefixes. The 8-rank fast path is expert-major; fallback is rank-major.
...
def dispatch_expert_prepare(
    ...
    Waits for the generation, caches inclusive per-(rank, expert) prefixes,
    writes the first half of masked_m, and returns total M plus max expert M.
    """
    assert sym_buf_addrs.dtype == torch.int64
    device = sym_buf_addrs.device
    if dbg_cyc is None:
        # An empty device tensor has a null data_ptr.  This disables the optional
        # clock writes without enqueueing a memset on every production launch.
        dbg_cyc = torch.empty(0, dtype=torch.int64, device=device)
    else:
        assert dbg_cyc.dtype == torch.int64 and dbg_cyc.numel() >= 4
...
def dispatch_expert_finalize(
    ...
    if dbg_cyc is None:
        ...
        assert dbg_cyc.dtype == torch.int64 and dbg_cyc.numel() >= 8
    ...
    staging_arg = _resolve_sfa_staging_addrs(
        staging_addrs, device, num_ranks,
        required=(os.getenv('DG_SFA_PUSH', '0') != '0'))
...
def dispatch_expert_preprocess_merged(
    ...
    assert sym_buf_addrs.dtype == torch.int64
    device = sym_buf_addrs.device
    profile_enabled = dbg_cyc is not None
    if dbg_cyc is None:
        dbg_cyc = torch.empty(0, dtype=torch.int64, device=device)
    else:
        assert dbg_cyc.dtype == torch.int64 and dbg_cyc.numel() >= 8
...
def dispatch_expert_sfa_overlap(
    sym_buf_addrs: torch.Tensor,
    rank_idx: int,
    num_ranks: int,
    num_local_experts: int,
    ...
    """Overlap metadata generation with dynamic-count pushed-SFA reshape.

    Counts and their rank prefixes are derived directly from the producer
    buffers, so SFA reshape does not wait for materialized GEMM metadata.  The
    first implementation is shape-specialized but supports arbitrary routing.
    """
    if (num_ranks, num_local_experts, num_total_experts,
            max_tokens_per_expert, hidden_dim, block_m) != (8, 12, 96, 256, 7168, 128):
        raise ValueError(
            "DG_SFA_OVERLAP requires nr=8, nle=12, nte=96, max_tokens=256, "
            "hidden=7168, block_m=128")
    if generation <= 0:
        raise ValueError("DG_SFA_OVERLAP requires generation > 0")
    if _workspace is None:
        raise ValueError("SFA overlap workspace is required")
    assert sym_buf_addrs.dtype == torch.int64
    assert local_sfa_buf.dtype == torch.uint16
    assert copy_ready_flags.dtype == torch.int32

    device = sym_buf_addrs.device
    staging_arg = _resolve_sfa_staging_addrs(
        staging_addrs, device, num_ranks, required=True)
    grouped_layout = _workspace.grouped_layout
    rank_addr_a = _workspace.rank_addr_a
    rank_addr_sfa = _workspace.rank_addr_sfa
    rank_split_m = _workspace.rank_split_m
    ...
    args = (sym_buf_addrs, pair_counts, grouped_layout, rank_addr_a, rank_addr_sfa,
            rank_split_m, rank_counts, masked_m, out_total_m_blocks, out_shape_m,
            out_expected_m, local_sfa_buf, copy_ready_flags,
            int(copy_ready_flags.numel()), generation, staging_arg,
            torch.cuda.current_stream())
    runtime = jit_tuner.compile_and_tune(
        name='dispatch_expert_sfa_overlap',
        keys={
            'RANK_IDX': rank_idx,
            'NUM_RANKS': num_ranks,
            'NUM_LOCAL_EXPERTS': num_local_experts,
        ...
        arg_defs=(
            ...
            ('num_copy_ready_flags', int),
            ('generation', int),
            ('staging_addrs', torch.int64),
            ('stream', torch.cuda.Stream),
        ),
        template=template_expert_sfa_overlap,
        args=args,
        jit_include_dir='cutlass3',
    )
    runtime(*args)
    ...
    if sync:
        ...
        total_m_blocks = num_local_experts * ceil_div(max_tokens_per_expert, block_m)
        shape_m = num_local_experts * max_tokens_per_expert
...
def dispatch_sfa_preprocess(
    ...
    args = (grouped_layout, rank_addr_sfa, rank_split_m, rank_counts, local_sfa_buf,
            copy_ready_flags, int(copy_ready_flags.numel()),
...
def fused_dispatch_block_copy_gemm1_fp4(
    ...
    *,
    ...
    sfa_preprocessed: bool = False,
    sfa_source_host: bool = None,
    exact_grid: bool = None,
    ...
    if exact_grid is None:
        # The validated 8-GPU production shape is 35% slower when the fused
        # launch is oversubscribed. Make its measured best launch semantics the
        # API default; explicit False remains available for A/B and other shapes
        # retain the environment-controlled legacy behavior.
        is_prod_8gpu = (
            num_ranks == 8 and num_groups == 12 and n == 6144 and k * 2 == 7168)
        exact_grid = (is_prod_8gpu or
                      os.getenv('FUSED_EXACT_GRID', '0') == '1')
    ...
    if merged_sfa_addrs is None:
        # GPU-SFA mode never dereferences merged_sfa_addrs.  rank_addr_sfa is an
        # existing contiguous int64 device tensor and is therefore a valid dummy
        # argument; reusing it avoids a per-call aten::zeros elementwise kernel.
        merged_sfa_addrs = rank_addr_sfa
    ...
    copy_flags_zeroed_by_preprocess = False
    if sfa_preprocessed:
        if sfa_source_host or os.getenv('DG_SFA_PUSH', '0') == '0':
            raise ValueError("sfa_preprocessed requires DG_SFA_PUSH and GPU SFA source")
        skip_sfa_copy = True
        copy_flags_zeroed_by_preprocess = True
    elif (os.getenv('DG_SFA_PREPROCESS', '0') != '0'
        ...
        if os.getenv('DG_SFA_PUSH_SKIP_RESHAPE', '0') == '0':  # diagnostic: skip reshape to isolate its cost
            sfa_num_sms = int(os.getenv('DG_SFA_NUM_SMS', str(num_sms)))
            sfa_num_threads = int(os.getenv('DG_SFA_NUM_THREADS', '256'))
            if not (1 <= sfa_num_sms <= num_sms):
                raise ValueError(f"DG_SFA_NUM_SMS must be in [1, {num_sms}]")
            if sfa_num_threads < 32 or sfa_num_threads > 1024 or sfa_num_threads % 32:
                raise ValueError("DG_SFA_NUM_THREADS must be a warp-aligned value in [32, 1024]")
            dispatch_sfa_preprocess(
                ...
                max_tokens_per_expert=max_tokens_per_expert,
                num_sms=sfa_num_sms, num_threads=sfa_num_threads)
            copy_flags_zeroed_by_preprocess = True
        skip_sfa_copy = True

    # In the production SFA-preprocess path the same-stream kernel clears these
    # flags while its other CTAs reshape SFA, avoiding a standalone memset. Keep
    # the original behavior for host-SFA and diagnostic skip-reshape paths.
    if not copy_flags_zeroed_by_preprocess:
    ...
    args = (rhs, rhs_scales, bias, out, gemm_shape_m,
            ...
            int(copy_only), int(skip_sfa_copy), int(sfa_source_host), int(exact_grid))
...
class BlockCopyDispatchContext:
    ...
    ordered on the same double-buffer lifetime. When ``DG_SFA_PUSH=1``, callers
    must also provide the separate symmetric SFA staging tensor and peer addresses;
    the context retains both for the full lifetime of every submitted round.
    """
    ...
    def __init__(
        ...
        local_expert_start: int,
        group=None,
        sfa_staging_buf: torch.Tensor = None,
        sfa_staging_addrs: torch.Tensor = None,
    ):
        ...
        push_enabled = os.getenv('DG_SFA_PUSH', '0') != '0'
        if push_enabled and (sfa_staging_buf is None or sfa_staging_addrs is None):
            raise ValueError(
                "DG_SFA_PUSH requires sfa_staging_buf and sfa_staging_addrs")
        if sfa_staging_buf is not None:
            if sfa_staging_buf.dtype != torch.uint8 or not sfa_staging_buf.is_contiguous():
                raise ValueError("sfa_staging_buf must be a contiguous uint8 tensor")
            if sfa_staging_buf.device != sym_buf.device:
                raise ValueError("sfa_staging_buf must be on the same device as sym_buf")
        if sfa_staging_addrs is not None:
            _resolve_sfa_staging_addrs(
                sfa_staging_addrs, sym_buf.device, num_ranks, required=push_enabled)
        self._sym_buf = sym_buf
        self._sym_buf_addrs = sym_buf_addrs
        self._sfa_staging_buf = sfa_staging_buf
        self._sfa_staging_addrs = sfa_staging_addrs
    ...
    def run(
        ...
        if self._sfa_staging_buf is not None:
            required_staging = get_sfa_staging_size(
                self._num_local_experts, self._num_total_experts,
                self._max_tokens, input_bf16.shape[1])
            if self._sfa_staging_buf.numel() < required_staging:
                raise ValueError(
                    f"sfa_staging_buf has {self._sfa_staging_buf.numel()} bytes, "
                    f"requires {required_staging}")
        ...
        host_profile = os.getenv('CONTEXT_HOST_PROFILE', '0') == '1'
        if host_profile:
        ...
        _mxfp4_quantize_to_sym_buffer(
            ...
            staging_addrs=self._sfa_staging_addrs)
        if host_profile:
            host_t_quant = time.perf_counter_ns()
        ...
        if host_profile:
            host_t_prepare = time.perf_counter_ns()
        configs = self._select_configs(shape_m, expected_m, n, k)
        block_m = configs[1]
        if host_profile:
            host_t_config = time.perf_counter_ns()
        # Production default for the only configuration validated by the block-copy
        # optimization campaign. Other shapes retain the conservative legacy defaults;
        # explicit caller values always win.
        is_prod_8gpu = (
            self._num_ranks == 8 and self._num_local_experts == 12
            and input_bf16.shape[1] == 7168 and n == 6144)
        if num_copy_blocks is None:
            num_copy_blocks = 3 if is_prod_8gpu else 1
        if k_tiles_per_flag is None:
            k_tiles_per_flag = 14 if is_prod_8gpu else 0
        num_sms, _, _, block_k, *_ = configs
        if num_copy_blocks < 0 or num_copy_blocks >= num_sms:
            raise ValueError(
                f"num_copy_blocks must be in [0, {num_sms}), got {num_copy_blocks}")
        if (k_tiles_per_flag < 0 or
                (k_tiles_per_flag > 0 and k % (block_k * k_tiles_per_flag) != 0)):
            raise ValueError(
                f"k_tiles_per_flag={k_tiles_per_flag} must evenly divide "
                f"the {k // block_k} K tiles")
        grouped_layout, rank_addr_a, rank_addr_sfa, rank_split_m, rank_counts, \
            _, finalized_shape_m = dispatch_expert_finalize(
                ...
                staging_addrs=self._sfa_staging_addrs)
        if host_profile:
            host_t_finalize = time.perf_counter_ns()
        ...
        if host_profile:
            host_t_buffers = time.perf_counter_ns()
        ...
        if host_profile:
            host_t_gemm = time.perf_counter_ns()
            ns_to_us = 1e-3
            print(
                f"[CONTEXT_HOST_PROFILE] rank={self._rank_idx} gen={generation} "
                f"quant={(host_t_quant-host_t0)*ns_to_us:.1f} "
                f"prepare={(host_t_prepare-host_t_quant)*ns_to_us:.1f} "
                f"config={(host_t_config-host_t_prepare)*ns_to_us:.1f} "
                f"finalize={(host_t_finalize-host_t_config)*ns_to_us:.1f} "
                f"buffers={(host_t_buffers-host_t_finalize)*ns_to_us:.1f} "
                f"gemm={(host_t_gemm-host_t_buffers)*ns_to_us:.1f} "
                f"total={(host_t_gemm-host_t0)*ns_to_us:.1f}", flush=True)
