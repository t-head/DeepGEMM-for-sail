...
gemm_t::run((const ElementA*) lhs, rhs, (const ElementA*) rhs_scales, (ElementA*) out,
            m, expected_m, stream, num_sms,
            m_rows, {topk, expert_ids_and_offset, sorted_token_ids, aligned_num_m_blocks});
"""
...
def m_grouped_gemm_w4a16_common(gemm_type: str, expected_m: int,
    ...
    if gemm_type == "GroupedNoPad":
        ...
        runtime = jit_tuner.compile_and_tune(
            ...
            keys={'N': n, 'K': k, 'BLOCK_M': block_m, 'BLOCK_N': block_n, 'BLOCK_K': block_k,
                ...
                'NUM_STAGES': num_stages, 'GROUP_SIZE': group_size, 'GEMM_TYPE': gemm_type, 'N_EXPAND': n_expand},
            space=(),
            includes=includes,
    ...
    elif gemm_type == "GroupedFused":
        topk, expert_ids_and_offset, sorted_token_ids, aligned_num_m_blocks = scheduler_extra
        args = (lhs, rhs, rhs_scales, out, m, expected_m, torch.cuda.current_stream(), num_sms,
                m_rows, topk, expert_ids_and_offset, sorted_token_ids, aligned_num_m_blocks)
        runtime = jit_tuner.compile_and_tune(
            name='m_grouped_gemm_w4a16',
            ...
            arg_defs=(('lhs', torch.bfloat16), ('rhs', torch.int32), ('rhs_scales', torch.bfloat16), ('out', torch.bfloat16),
                    ('m', int), ('expected_m', int), ('stream', torch.cuda.Stream), ('num_sms', int),
                    ('m_rows', torch.int32), ('topk', int),
                    ('expert_ids_and_offset', torch.int32), ('sorted_token_ids', torch.int32), ('aligned_num_m_blocks', torch.int32)),
...
def m_grouped_gemm_w4a16_fused(lhs: torch.Tensor,
                                ...
                                out: torch.Tensor,
                                m_rows: torch.Tensor,
                                expert_ids_and_offset: torch.Tensor,
    ...
    m_grouped_gemm_w4a16_common("GroupedFused", expected_m, lhs, rhs_, out, group_size, configs, m_rows, (topk, expert_ids_and_offset, sorted_token_ids, aligned_num_m_blocks))
...
def m_grouped_gemm_w4a16_masked(lhs: torch.Tensor,
    ...
    if configs is None:
        configs = get_w4a16_config("GroupedMasked", expected_m, n, k, group_size)
    m_grouped_gemm_w4a16_common("GroupedMasked", expected_m, lhs, rhs_, out, group_size, configs, masked_m, None)
...
def m_grouped_gemm_w4a16_nopad(lhs: torch.Tensor,
    ...
    m_grouped_gemm_w4a16_common("GroupedNoPad", expected_m, lhs, rhs_, out, group_size, configs, m_rows, block_m_info)
