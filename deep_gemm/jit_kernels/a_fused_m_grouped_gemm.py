...
includes_fusedmoe_gemm_with_blkwise_quant = ('"../deep_gemm/fused_moe_gemm_with_blkwise_quant.cuh"', )
...
def m_grouped_gemm_fp8_fp8_bf16_nt_fused(lhs_: Tuple[torch.Tensor],
    ...
    assert lhs.is_contiguous() and rhs.is_contiguous()
    assert out.is_contiguous()
    assert k % 16 == 0, "K must be a multiple of 16 so that 16 fp8 elements can be loaded with 128b aligned vectorized memory access."
