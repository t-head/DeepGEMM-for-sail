...
genc_map = {
    ...
    torch.uint16: ('void*', 'uint16_t*'),
    torch.int64: ('void*', 'int64_t*'),
    torch.float: ('void*', 'float*'),
    torch.bfloat16: ('void*', '__ppu_bfloat16*'),
    torch.float8_e4m3fn: ('void*', '__hg_fp8_e4m3*'),
    torch.cuda.Stream: ('void*', 'hggcStream_t'),
}
...
def generate(includes: Iterable[str], arg_defs: Iterable[Tuple], body: str) -> str:
    # Common prefix
    code = '// DeepGEMM auto-generated JIT source file\n\n'

    # Includes (PPU SDK headers)
    preload_sys_includes = ['<hggc.h>', '<hggc_fp8.h>', '<hggc_runtime.h>', '<iostream>']
