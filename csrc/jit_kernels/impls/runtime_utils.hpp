...
static HGtensorMapDataType aten_dtype_to_tensor_map_dtype(const at::ScalarType& dtype,
                                                          const bool& allow_tf32) {
    if (allow_tf32 and dtype == torch::kFloat)
        return HG_TENSOR_MAP_DATA_TYPE_TFLOAT32;

    switch (dtype) {
        case torch::kInt:           return HG_TENSOR_MAP_DATA_TYPE_INT32;
        case torch::kFloat:         return HG_TENSOR_MAP_DATA_TYPE_FLOAT32;
        case torch::kBFloat16:      return HG_TENSOR_MAP_DATA_TYPE_BFLOAT16;
        case torch::kFloat8_e4m3fn: return HG_TENSOR_MAP_DATA_TYPE_UINT8;
        default: DG_HOST_UNREACHABLE("Unsupported dtype");
    }
}

static HGtensorMapSwizzle mode_into_tensor_map_swizzle(const int& mode, const int& base) {
#if HGGCRT_VERSION >= 12080
    if (base != 0) {
        DG_HOST_ASSERT(base == 32 and mode == 128);
        return HG_TENSOR_MAP_SWIZZLE_128B_ATOM_32B;
    }
#endif
    ...
    switch (mode) {
        case   0:
        case  16: return HG_TENSOR_MAP_SWIZZLE_NONE;
        case  32: return HG_TENSOR_MAP_SWIZZLE_32B;
        case  64: return HG_TENSOR_MAP_SWIZZLE_64B;
        case 128: return HG_TENSOR_MAP_SWIZZLE_128B;
        default: DG_HOST_UNREACHABLE("Unsupported swizzling mode");
    }
}

static HGtensorMap make_tma_2d_desc(const torch::Tensor& t,
    ...
    HGtensorMap tensor_map;
    const hguint64_t gmem_dims[2] = {static_cast<hguint64_t>(gmem_inner_dim), static_cast<hguint64_t>(gmem_outer_dim)};
    const hguint32_t smem_dims[2] = {static_cast<hguint32_t>(smem_inner_dim), static_cast<hguint32_t>(smem_outer_dim)};
    const hguint64_t gmem_strides[1] = {static_cast<hguint64_t>(gmem_outer_stride * elem_size), };
    const hguint32_t elem_strides[2] = {1, 1};
    ...
    DG_HGGC_DRIVER_CHECK(lazy_hgTensorMapEncodeTiled(
        ...
        HG_TENSOR_MAP_INTERLEAVE_NONE, mode_into_tensor_map_swizzle(swizzle_mode, swizzle_base),
        HG_TENSOR_MAP_L2_PROMOTION_L2_256B, HG_TENSOR_MAP_FLOAT_OOB_FILL_NONE));
    return tensor_map;
}

static HGtensorMap make_tma_3d_desc(const torch::Tensor& t,
    ...
    HGtensorMap tensor_map;
    const hguint64_t gmem_dims[3] = {static_cast<hguint64_t>(gmem_dim_0), static_cast<hguint64_t>(gmem_dim_1), static_cast<hguint64_t>(gmem_dim_2),};
    const hguint32_t smem_dims[3] = {static_cast<hguint32_t>(smem_dim_0), static_cast<hguint32_t>(smem_dim_1), static_cast<hguint32_t>(smem_dim_2)};
    const hguint64_t gmem_strides[2] = {static_cast<hguint64_t>(gmem_stride_0 * elem_size), static_cast<hguint64_t>(gmem_stride_1 * elem_size)};
    const hguint32_t elem_strides[3] = {1, 1, 1};
    if (get_env<int>("DG_JIT_DEBUG")) {
...
static HGtensorMap make_tma_a_desc(const cute::UMMA::Major& major,
...
static HGtensorMap make_tma_b_desc(const cute::UMMA::Major& major,
                                   const torch::Tensor& t,
...
static HGtensorMap make_tma_cd_desc(const torch::Tensor& t,
...
static HGtensorMap make_tma_sf_desc(const cute::UMMA::Major& major,
                                    const torch::Tensor& t,
