...
public:
    ...
    std::vector<std::vector<int>> _add_candidate_tile(
        const std::vector<std::vector<int>>& candidate_tile, 
        const std::vector<int>& new_tile, 
        int compare_index = -1, 
        int top_k = 10, 
        ...
        if (tile_map.find(new_tile[compare_index]) != tile_map.end()) {
            ...
            auto it = std::find(candidate_tile_copy.begin(), candidate_tile_copy.end(), tile_map[new_tile[compare_index]].back());
    ...
    std::vector<std::vector<int>> _get_collapsed_topk(
        const std::vector<std::vector<int>>& candidate_tile, 
        int compare_index = -1, 
        int top_k = 10, 
        bool ascend_order = true) {
    ...
    double _get_l2_usage(int bm, int bn, int bk, int stages, int occ) {
        return ((bm*bk + bn*bk) * _bpp * stages * occ * _hw_metric.cu_per_ce) / static_cast<double>(_hw_metric.l2_size);
    }
    ...
    double _get_llc_usage(int bm, int bn, int bk, int stages, int occ) {
        return (((bm*bk + bn*bk) * _bpp * stages + bm*bn*_bpp) * occ * _hw_metric.cu_count) / static_cast<double>(_hw_metric.llc_size);
    }
    ...
    std::vector<std::vector<int>> _get_block_tile(int m, int n, int k, 
                                                 const std::vector<std::vector<int>>& tile_list, 
                                                 int top_k = 50) {
        std::vector<std::vector<int>> new_tile_candidate;
    ...
    double _get_regs_rloadsize_per_cu(int bm, int bn, int bk, int wm, int wn) {
        ...
        double rload_data_per_block = (wm*bk + wn*bk)*_bpp*(bm/wm)*(bn/wn) / static_cast<double>((bm*bk+bn*bk)*_bpp);
        ...
        for (auto tile : tile_list) {
            ...
            for (int wm : wm_factors) {
                for (int wn : wn_factors) {
                    ...
                    if (!(_hw_metric.min_warp_per_block <= warps_per_block && warps_per_block <= _hw_metric.max_warp_per_block)) {
                        continue;
                    }
    ...
    std::vector<std::vector<int>> _multi_phase_topk(std::vector<std::vector<int>> new_tile_candidate, int num_candidate) {
    ...
    std::vector<std::vector<int>> _get_knowledge_base() {
        std::vector<std::vector<int>> shape_knowledge = {{4096, 4608, 7168},
                                                        {4096, 36864, 7168},
                                                        {4096, 7168, 16384},
                                                        {4096, 7168, 18432},
                                                        {4096, 4096, 7168}};
    ...
    void output_csv(const std::vector<std::vector<int>>& candidate_tile_list, const std::string& file_name = "tiling_result") {
        std::string result_file = file_name + ".csv";
        std::vector<std::string> header = {"m","n","k","bm","bn","bk","wm","wn","wk","stages","cu_count",
