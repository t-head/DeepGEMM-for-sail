...
public:
    ...
    explicit KernelRuntime(const std::filesystem::path& dir_path) {
        ...
        const auto& cuobjdump_path = cuda_home / "../bin" / "hgobjdump";
        ...
        kernel = load_kernel(cubin_path, expected_name, &library);
    }
...
public:
    ...
    template <typename Args>
    static void launch(const std::shared_ptr<KernelRuntime>& kernel_runtime, const Args& args, at::cuda::CUDAStream stream=nullptr) {
        ...
        if (get_env<int>("DG_JIT_DEBUG")) {
            printf("Launch kernel with {%d, %d} x %d  %d, shared memory: %d bytes, stream: %ld\n",
                   ...
                   launch_args.smem_size, stream.id());
        }
