...
- **HGCC/HGRTC selections**
  - `DG_JIT_USE_NVRTC`: `0` or `1`, use HGRTC instead of HGCC, faster compilation but maybe have lower performance for some cases, `0` by default
  - `DG_JIT_NVCC_COMPILER`: string, specified compiler path; will find in `torch.utils.cpp_extension.CUDA_HOME` by default
- **Compiler options**
...
Following the CUTLASS design, the kernels in DeepGEMM for PPU adopt warp interleave design, enabling overlapping data movement, MMA instructions, and promotion operations.
